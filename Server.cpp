...
private:
	void handle_write(const boost::system::error_code& error, size_t bytes_transferred)
	{
...

		{
				...
				boost::asio::async_write(socket_, boost::asio::buffer(v1, bytes_transferred),
					boost::bind(&session::handle_write, this,
...
int main()
{
	...
	// function for data string to generate random packet and give vector
	/*vector<string> data = { "JKLrrr", "alphaaa", "betaaa", "gammaa", "ashhhhhh", "pack234", "packetabc", "packet234", "packe1hz", "mypacket" };
	vector <uint8_t> v;
		for (int i = 0; i < 20; i++) {
			v = get_random_packet(data);
